...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const accessToken = jwt.sign({ userId: user._id, username: user.username, email: user.email, profilepicture: user.profilepicture, role: user.role }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '30m' });
